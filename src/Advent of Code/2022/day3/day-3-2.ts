...
type stringArrOfArrays = [] | string[][];

const round2 = round1.reduce<string[][]>(
  (returnArray, currentElf, elfIndex) => {
    if (elfIndex % 3 === 0) {
      returnArray.push([]);
    }
...
function findMatchInGroup(groupArray: string[]): string {
...
function findLetterNumber(letter: string): number {
  const lowerCase = "abcdefghijklmnopqrstuvwxyz";

...
function showResults(arr: expectedObj[]) {
  ...
  const elements = arr.reduce(
    (
      annotationAcc,
      { annotation, article, created, link, quote, tagsArray, updated }
    ) => {
      if (
        tagsArray.includes('#CITATION') ||
      ...
      const { emojis, tags } = tagsArray.reduce(
        (tagAcc, tagString) => {
          switch (tagString) {
            case '#IMP':
            case '#IMPORTANT': {
